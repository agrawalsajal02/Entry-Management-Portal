...
app.post("/checkin", function (req, res) {
  ...
  host.findOne({ name: userhost }, function (e, user) {
    ...
    if (user === null) {
      ...
      var msg = `${req.body.name.toUpperCase()}|${req.body.email}|${
        req.body.phone
      }|${tim}`;
      var fullmsg =
        "https://www.fast2sms.com/dev/bulk?authorization=ejdkgTpc0ZuCsH5vL7OFSRwPyaG6UMQtWlYhnIXmqoA9f2xJb8LrCxtquT71KJegEi2NYVvcbRnkSowh&sender_id=FSTSMS&language=english&route=qt&numbers=" +
        user.phone +
        "&message=19086&variables={DD}|{EE}|{CC}|{FF}&variables_values=" +
      ...
      visiter.create(newvisiter, function (err, person) {
        if (err) {
          ...
          <li>Check in time:  ${person.checkintime.toLocaleTimeString(
            "en-US"
          )}</li>
          </ul>
          <p>Please Call him</p>
...
app.get("/checkout/:id", function (req, res) {
  ...
  visiter.findById(id, function (err, guest) {
    if (err) {
      ...
      var tim = new Date();
      tim = tim.toLocaleTimeString("en-US");
      var msg = `${guest.name.toUpperCase()}|${
        guest.checkintime
      }|${tim}|${guest.host.toUpperCase()}|SummerGeek , Patna ,Bihar`;
      var fullmsg =
        "https://www.fast2sms.com/dev/bulk?authorization=ejdkgTpc0ZuCsH5vL7OFSRwPyaG6UMQtWlYhnIXmqoA9f2xJb8LrCxtquT71KJegEi2NYVvcbRnkSowh&sender_id=FSTSMS&language=english&route=qt&numbers=" +
        guest.phone +
        "&message=19085&variables={CC}|{BB}|{DD}|{FF}|{EE}&variables_values=" +
        encodeURIComponent(msg);
      ...
      const output = `
                  ...
                  <li>Check in time:  ${guest.checkintime.toLocaleTimeString(
                    "en-US"
                  )}</li>
